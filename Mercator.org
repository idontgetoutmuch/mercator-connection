...
#+begin_src lean4 :tangle no
def roundInner (x : sphSourceOpens) : TangentSpace (𝓡 2) x →L[ℝ] TangentSpace (𝓡 2) x →L[ℝ] ℝ :=
  dθ x ⊗ dθ x + (Real.sin (θ_coord x)) ^ 2 • dφ x ⊗ dφ x
...
First we give a helper lemma. The chart \(\chi : S^2
\to \mathbb{R}^2\) and its inverse \(\chi^{-1}\) are mutually inverse
on \(\text{sphSource}\), so \(\chi \circ \chi^{-1} =
\operatorname{id}\); differentiating this identity and applying the
chain rule, their differentials compose to the identity too:

\[
d\chi_x \circ d\chi^{-1}_{\chi(x)} = \operatorname{id}_{\mathbb{R}^2}.
\]
...
And so we obtain the duality relations
\[
