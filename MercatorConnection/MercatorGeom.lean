...
lemma φ_coord_contMDiffAt {x : S2} (hx : x ∈ sphSource) :
  ...
  have hfun : φ_coord = fun p : S2 => Complex.arg ((p.val 0 : ℂ) + (p.val 1 : ℂ) * Complex.I) := by
    funext p
    simp only [φ_coord, sphericalChart, OpenPartialHomeomorph.coe_mk, sphFwd, Fin.isValue, Complex.equivRealProd_symm_apply, WithLp.equiv_symm_apply, Matrix.cons_val_one, Matrix.cons_val_fin_one]
  rw [hfun]; exact h_arg
